...
class ImageGenService:
    ...
    def __init__(self, config: ImageGenConfig, *, api_key: Optional[str] = None) -> None:
        ...
        DebugLogger().log(f"AI Image Generation Service initialized with model: {self.config.model}", "info")
        DebugLogger().log(f"  • Portrait size: {self.config.portrait_size}", "debug")
        DebugLogger().log(f"  • Banner size: {self.config.banner_size}", "debug")
        DebugLogger().log(f"  • Quality: {self.config.quality}", "debug")
    ...
    def generate_for_recipe(self, recipe_name: str) -> ImagePairPaths:
        ...
        """
        DebugLogger().log(f"Starting image generation for '{recipe_name}' using model: {self.config.model}", "info")
        ...
        if (not self.config.allow_overwrite and
            ...
            DebugLogger().log(f"Images already exist for '{recipe_name}', skipping generation", "info")
        ...
        DebugLogger().log(f"Generating images using {self.config.model}: portrait ({self.config.portrait_size}) + banner ({self.config.banner_size})", "info")
        ...
        DebugLogger().log(f"Successfully generated and saved images for '{recipe_name}' using {self.config.model}", "info")
        DebugLogger().log(f"  • Portrait: {portrait_path.name}", "debug")
        DebugLogger().log(f"  • Banner: {banner_path.name}", "debug")
        ...
        async def gen_b64(prompt: str, size: str) -> bytes:
            ...
            for attempt in range(3):
                try:
                    ...
                    DebugLogger().log(f"Attempt {attempt + 1}/3 failed for {self.config.model} @ {size}: {e}", "warning")
                    if attempt == 2:
                        DebugLogger().log(f"All attempts failed for {self.config.model} @ {size}", "error")
                        raise
